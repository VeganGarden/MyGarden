...
echo -e "  - 部署后必须配置SPA路由支持，否则直接访问路由（如/login）会返回403/404错误"
echo -e "  - 请按照以下步骤配置错误页面重定向："
echo -e ""
echo -e "${BLUE}配置SPA路由步骤（必须配置！）：${NC}"
echo -e "  1. 访问腾讯云开发控制台：${GREEN}https://console.cloud.tencent.com/tcb${NC}"
echo -e "  2. 选择环境：${GREEN}${ENV_ID}${NC}"
echo -e "  3. 进入「静态网站托管」→ 「基础配置」"
echo -e "  4. 找到「错误页面」配置，点击「添加规则」或「配置错误页面」"
echo -e "  5. 配置如下（重要）："
echo -e "     - 错误码：${GREEN}404${NC}"
echo -e "     - 响应码：${GREEN}200${NC}（必须设置为200，保持浏览器URL不变）"
echo -e "     - 响应页面：${GREEN}/index.html${NC}"
echo -e "  6. 保存配置并等待 2-5 分钟生效"
echo -e "  7. 清除浏览器缓存后重新测试"
echo -e ""
echo -e "${RED}如果未配置此步骤，访问路由路径（如 /login）会出现 403 或 404 错误！${NC}"
echo -e "${RED}详细说明请查看：${GREEN}SPA_ROUTING_FIX.md${NC}"
echo -e ""
echo -e "${BLUE}💡 其他提示：${NC}"
