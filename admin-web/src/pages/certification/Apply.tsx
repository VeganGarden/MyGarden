...
const CertificationApply: React.FC = () => {
  ...
  const loadOperationData = async () => {
    ...
    try {
      ...
      // 1. 加载运营台账数据（能源使用和浪费管理）
      const ledgerResult = await operationAPI.ledger.list({
        restaurantId,
        tenantId,
        page: 1,
        pageSize: 100, // 获取所有记录
      ...
      let energyUsageText = ''
      let wasteReductionText = ''
      const socialInitiativesList: string[] = []

      if (ledgerResult && ledgerResult.code === 0 && ledgerResult.data) {
        const ledgerEntries = Array.isArray(ledgerResult.data) ? ledgerResult.data : []
        
        // 按类型分类处理
        const energyEntries = ledgerEntries.filter((entry: any) => entry.type === 'energy')
        const wasteEntries = ledgerEntries.filter((entry: any) => entry.type === 'waste')
        const trainingEntries = ledgerEntries.filter((entry: any) => entry.type === 'training')

        // 构建能源使用情况文本
        if (energyEntries.length > 0) {
          const energyParts = energyEntries.map((entry: any, index: number) => {
            const parts = []
            if (entry.date) parts.push(`日期: ${entry.date}`)
            if (entry.description) parts.push(`说明: ${entry.description}`)
            if (entry.value !== undefined && entry.unit) {
              parts.push(`数值: ${entry.value} ${entry.unit}`)
            }
            return parts.length > 0 ? `【记录${index + 1}】${parts.join('，')}` : null
          }).filter(Boolean)
          
          energyUsageText = energyParts.join('\n')
          
          // 添加汇总信息
          const totalEnergy = energyEntries.reduce((sum: number, entry: any) => sum + (entry.value || 0), 0)
          const energyUnit = energyEntries[0]?.unit || 'kWh'
          if (energyParts.length > 0) {
            energyUsageText += `\n\n【年度汇总】总能源消耗: ${totalEnergy} ${energyUnit}`
          }
        }

        // 构建食物浪费管理文本
        if (wasteEntries.length > 0) {
          const wasteParts = wasteEntries.map((entry: any, index: number) => {
            const parts = []
            if (entry.date) parts.push(`日期: ${entry.date}`)
            if (entry.description) parts.push(`说明: ${entry.description}`)
            if (entry.value !== undefined && entry.unit) {
          ...
          wasteReductionText = wasteParts.join('\n')
          
          // 添加汇总信息
          const totalWaste = wasteEntries.reduce((sum: number, entry: any) => sum + (entry.value || 0), 0)
          const wasteUnit = wasteEntries[0]?.unit || 'kg'
          if (wasteParts.length > 0) {
            wasteReductionText += `\n\n【月度汇总】总浪费量: ${totalWaste} ${wasteUnit}`
          }
        }
        ...
        if (trainingEntries.length > 0) {
          trainingEntries.forEach((entry: any) => {
            if (entry.description) {
              socialInitiativesList.push(`员工培训: ${entry.description}`)
            }
          })
        }
      }

      // 2. 尝试从行为统计获取社会传播与教育相关数据
      try {
        const behaviorResult = await operationAPI.behavior.getMetrics({
          restaurantId,
          tenantId,
        })

        if (behaviorResult && behaviorResult.code === 0 && behaviorResult.data) {
          // 如果有公益活动、顾客教育等数据，添加到社会传播与教育列表
          const behaviorData = behaviorResult.data
          
          // 示例：如果有相关字段，可以提取
          // if (behaviorData.publicActivities) {
          //   socialInitiativesList.push(...behaviorData.publicActivities)
          // }
        }
      } catch (error) {
        // 行为统计API可能不存在或失败，不影响主流程
        console.log('获取行为统计数据失败（可选）:', error)
      }
      ...
      if (energyUsageText) {
        operationValues.energyUsage = energyUsageText
      }
      
      if (wasteReductionText) {
        operationValues.wasteReduction = wasteReductionText
      }
      
      if (socialInitiativesList.length > 0) {
        operationValues.socialInitiatives = socialInitiativesList.join('\n')
      }
      ...
      if (Object.keys(operationValues).length > 0) {
        ...
        const loadedItems = []
        if (energyUsageText) loadedItems.push('能源使用情况')
        if (wasteReductionText) loadedItems.push('食物浪费管理')
        if (socialInitiativesList.length > 0) loadedItems.push('社会传播与教育')
        
        message.success(`已加载运营数据：${loadedItems.join('、')}`)
      } else {
        message.warning('运营管理模块暂无相关数据，请手动填写或先在运营管理模块录入数据')
      }
    } catch (error: any) {
